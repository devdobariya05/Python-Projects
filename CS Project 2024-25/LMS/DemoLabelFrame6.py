...
class SMS:
    ...
    def __init__(self,root):
        ...
        self.var6 = StringVar()
        #=============================== Label  ================================================#
        lblTitle=Label(self.root,text="J. B. & KARP Vidya Sankul",bg="sky blue",fg="green",bd=5,relief=GROOVE,\
        ...
        self.lb1_hr = Label(self.root, text='12', font=('times new roman', 20, 'bold'), bg='sky blue', fg='white',bd=2,relief=GROOVE)
        self.lb1_hr.place(x=1100, y=5, width=40, height=30)
        
        self.lb2_hr = Label(self.root, text=':', font=('times new roman', 20, 'bold'), bg='sky blue', fg='white',bd=2,relief=GROOVE)
        self.lb2_hr.place(x=1140, y=5, width=40, height=30)
        
        self.lb3_hr = Label(self.root, text='05', font=('times new roman', 20, 'bold'), bg='sky blue', fg='white',bd=2,relief=GROOVE)
        self.lb3_hr.place(x=1180, y=5, width=40, height=30)
        
        self.lb4_hr = Label(self.root, text=':', font=('times new roman', 20, 'bold'), bg='sky blue', fg='white',bd=2,relief=GROOVE)
        self.lb4_hr.place(x=1220, y=5, width=40, height=30)
        
        self.lb5_hr = Label(self.root, text='37', font=('times new roman', 20, 'bold'), bg='sky blue', fg='white',bd=2,relief=GROOVE)
        self.lb5_hr.place(x=1260, y=5, width=40, height=30)
       
        self.lb7_hr = Label(self.root, text='AM', font=('times new roman', 20, 'bold'), bg='sky blue', fg='white',bd=2,relief=GROOVE)
        self.lb7_hr.place(x=1300, y=5, width=40, height=30)

        date=dt.datetime.now()
        self.lb8_hr = Label(self.root, text=f"{date:%A, %B %d, %Y}",font=('times new roman', 20, 'bold'), bg='sky blue', fg='white',bd=2,relief=RAISED)
        self.lb8_hr.place(x=1095, y=35, width=450, height=30)

        self.lb9_hr = Label(self.root, text="Designed By:Ajay Tiwari IT Solution", font='arial 10', bg='sky blue', fg='maroon',bd=2,relief=GROOVE)
        self.lb9_hr.place(x=10, y=5, width=350, height=30)
        self.lb10_hr = Label(self.root, text="J.B. & KARP", font='arial 10', bg='sky blue', fg='maroon',bd=2,relief=SUNKEN)
        self.lb10_hr.place(x=10, y=35, width=350, height=30)
        ...
        cmbMember=ttk.Combobox(DataFrameLeft,textvariable=self.var1,font=("times new roman",13,"bold"),width=15,state="readonly")
        ...
        txtFirstName=Entry(DataFrameLeft, textvariable=self.var2,font=("times new roman",13),width=17)
        ...
        rd1 = Radiobutton(DataFrameLeft ,bg="powder blue",text="Male" ,padx = 5,fg="black",bd=0,width=8,font=("arial",10,'bold'),relief = 'raise',variable = self.var3, value = "Male")
        rd1.grid(row=2,column=1,sticky=W)

        rd2 = Radiobutton(DataFrameLeft ,bg="powder blue",text="Female" ,padx = 10,fg="black",bd=0,width=8,font=("arial",10,'bold'),relief = 'raise',variable = self.var3, value = "Female")
        ...
        chk1 = Checkbutton(DataFrameLeft ,bg="powder blue",text="English" ,padx = 10,fg="black",bd=0,width=8,font=("arial",10,'bold'),relief = 'raise',variable = self.var4)
        chk1.grid(row=3,column=1,sticky=W)

        chk2 = Checkbutton(DataFrameLeft ,bg="powder blue",text="Hindi" ,padx = 10,fg="black",bd=0,width=8,font=("arial",10,'bold'),relief = 'raise',variable = self.var5)
        ...
        def SelectMember(event=""):                       #To add selected book from Book List to the controls in Left Frame
            val=str(lstBox.get(lstBox.curselection()))
            if(val=="Administrator"):
                self.var1.set("Administrator")
                self.var2.set("Gaurang")
                self.var3.set("Male")
                self.var4.set("0")
                self.var5.set("")
                self.var6.set("Surat")
            elif(val=="Teacher"):
                self.var1.set("Teacher")
                self.var2.set("Heena")
                self.var3.set("Female")
                self.var4.set("")
                self.var5.set("0")
                self.var6.set("Surat")
            elif(val=="Student"):
                self.var1.set("Student")
                self.var2.set("Prem")
                self.var3.set("Male")
                self.var4.set("")
                self.var5.set("")
                self.var6.set("Valsad")
            else:
                pass
            
        lstBox=Listbox(DataFrameRight,font=("arial",12,"bold"),width=30,height=15,bd=5)
        lstBox.bind("<<ListboxSelect>>",SelectMember)
        ...
        self.Table.column("Member",width=50)                   #Fixing the width of all fields
        self.Table.column("FirstName",width=50)
        self.Table.column("Gender",width=50)
        self.Table.column("Langauge1",width=50)
        self.Table.column("Langauge2",width=50)
...
##        lbl.config(image=self.ig)
    ...
    def add(self):
        mydb=sql.connect(host="localhost",user="root",passwd="12345",database="mySMS")
        mycur=mydb.cursor()
        mycur.execute("insert into member values(%s,%s,%s,%s,%s,%s)",(
                                                                        self.var1.get(),
                                                                        self.var2.get(),
                                                                        self.var3.get(),
                                                                        self.var4.get(),
    ...
    def fetch_data(self):           #Function to access all records from the bookdetails table
        mydb=sql.connect(host="localhost",user="root",passwd="12345",database="mySMS")
        mycur=mydb.cursor()
        mycur.execute("select * from member")
        rows=mycur.fetchall()
    ...
    def get_cursor(self,event=""):           #To focus the curosr on the table
        ...
        row=content["values"]
        self.var1.set(row[0]),    #To show the values from table to their respective field in the second frame.
        self.var2.set(row[1]),
        self.var3.set(row[2]),
        self.var4.set(row[3]),
        self.var5.set(row[4]),
    ...
    def show(self):
        self.txtBox.insert(END,"Member Type:\t\t"+self.var1.get()+"\n")
        self.txtBox.insert(END,"First Name:\t\t"+self.var2.get()+"\n")
        self.txtBox.insert(END,"Gender:\t\t"+self.var3.get()+"\n")
        self.txtBox.insert(END,"Language1:\t\t"+self.var4.get()+"\n")
        self.txtBox.insert(END,"Language2:\t\t"+self.var5.get()+"\n")
        self.txtBox.insert(END,"Address:\t\t"+self.var6.get()+"\n")
        
    def update(self):
        mydb=sql.connect(host="localhost",user="root",passwd="12345",database="mySMS")
        mycur=mydb.cursor()
        mycur.execute("update member set Member=%s,Gender=%s,Language1=%s,Language2=%s,Address=%s where FirstName=%s",(
                                                         self.var1.get(),
                                                         self.var3.get(),
                                                         self.var4.get(),
                                                         self.var5.get(),
    ...
    def delete(self):
        if self.var1.get()=="" or self.var2.get()=="":
            ...
            mydb=sql.connect(host="localhost",user="root",passwd="12345",database="mySMS")
            mycur=mydb.cursor()
            query="delete from member where FirstName=%s"
            value=(self.var2.get(),)
    ...
    def iexit(self):
        iexit=tkinter.messagebox.askyesno("Member Management System","Do you want to exit?")
        if iexit>0:
            self.root.destroy()
            return
