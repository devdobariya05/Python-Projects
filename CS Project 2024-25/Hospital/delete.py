...
def Viewregist():
    view_r = Toplevel()
    view_r.title('Delete ')
    view_r.geometry('1260x630')
    image1 = ImageTk.PhotoImage(Image.open("bg.png"))
    image1_label = Label(view_r,image=image1)     
    image1_label.place(x=0, y=0)
    delbut1=ImageTk.PhotoImage(Image.open("delbut.png"))

    #---------------------Label Frame------------------------

    labelframe=LabelFrame(view_r,bg='white',width=500,height=550,borderwidth=5, relief="groove")
    labelframe.place(x=350,y=50)
    ...
    def delete():
        ...
        ilast = last_e.get()
        cur = mydb.cursor()
        cur.execute("DELETE FROM hospital WHERE name = %s and last = %s ",(iname, ilast))
        mydb.commit()
        mydb.close()
    ...
    def submit():
        ...
        #------------------Image Label-----------------------
        labelframe=LabelFrame(view,bg='white',width=500,height=600,borderwidth=5, relief="groove")
        labelframe.place(x=350,y=50)
        ...
        dname = cur.fetchone()
        cur.execute("select last from hospital where name=%s and last=%s ", (iname, ilast))
        dlast = cur.fetchone()
        cur.execute('select age from hospital where name=%s and last=%s ',(iname, ilast))
        dage = cur.fetchone()
        cur.execute('select gender from hospital where name=%s and last=%s',(iname, ilast))
        dgender = cur.fetchone()
        cur.execute('select phone from hospital where name=%s and last=%s ',(iname, ilast))
        dphone = cur.fetchone()
        cur.execute('select appt from hospital where name=%s and last=%s',(iname, ilast))
        dappt = cur.fetchone()
        ...
        Button(labelframe,image=back_button,bd=0,bg='white', command=view.destroy).place(x=150, y=500)
        Button(labelframe, bg='white', text='del',image=delbut1,bd=0,command=delete).place(x=300, y=500)
